...
class SessionGraph(Module):
    def __init__(self, opt, n_node):
        ...
        self.pos_embedding = nn.Embedding(50, self.hidden_size)
    ...
    def forward(self, inputs, mask, mask_repeat):
        ...
        ht = init_hidden[torch.arange(mask.shape[0]).long(), torch.sum(mask.long(), 1) - 1]

        # Pos Embedding
        bs, item_num = init_hidden.shape[0], init_hidden.shape[1]
        index = torch.arange(item_num).unsqueeze(0) # 1 * item_num
        pos_index = index.repeat(bs, 1).view(bs, item_num) # bs * item_num
        pos_index = trans_to_cuda(torch.Tensor(pos_index.float()).long())
        pos_hidden = self.pos_embedding(pos_index)
        hidden = init_hidden + pos_hidden

        alpha = self.importance_extraction(init_hidden, mask, mask_repeat)
